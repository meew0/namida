...
pub unsafe extern "C" fn accept_block(
    ...
    let mut ringbuf_pointer: u_int32_t = 0;
    ...
    ringbuf_pointer = block_index.wrapping_sub(1 as libc::c_int as u_int32_t)
        % 1 as libc::c_int as u_int32_t * (*(*session).parameter).block_size;
    if !((*(*session).parameter).ringbuf).is_null() {
        memcpy(
            ((*(*session).parameter).ringbuf).offset(ringbuf_pointer as isize)
                as *mut libc::c_void,
            block as *const libc::c_void,
            write_size as libc::c_ulong,
        );
    }
    write_vsib_block(session, block, write_size as size_t);
    status = fseeko(
        (*transfer).file,
