...
pub unsafe extern "C" fn ttp_open_transfer(
    ...
    (*param)
        .ringbuf = malloc(
        ((*param).block_size * 1 as libc::c_int as u_int32_t) as libc::c_ulong,
    ) as *mut libc::c_char;
    if ((*param).ringbuf).is_null() {
        return error_handler(
            b"protocol.c\0" as *const u8 as *const libc::c_char,
            247 as libc::c_int,
            b"Could not reserve space for ring buffer\0" as *const u8
                as *const libc::c_char,
            0 as libc::c_int,
        );
    }
...
pub unsafe extern "C" fn ttp_request_stop(
    ...
    stop_vsib(session);
    free((*(*session).parameter).ringbuf as *mut libc::c_void);
    (*(*session).parameter).ringbuf = 0 as *mut libc::c_char;
    return 0 as libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn ttp_update_stats(
    ...
    sprintf(
        ...
        b"%02d:%02d:%02d.%03d %4u %6.2fM %6.1fMbps %5.1f%% %7u %6.1fG %6.1fMbps %5.1f%% %5d %5d %7u %8u %8Lu\n\0"
            as *const u8 as *const libc::c_char,
        hours,
        minutes,
        seconds,
